...
class AnnovarWithHail: 
    ...
    def filtering_myvcf(self, matrixtable, gnomadCutoff, GQCutoff, DPCutoff, PLDIFFDPCutoff):
        mt = matrixtable
        ...
        mt = mt.filter_rows((mt.gnomAD_r211_merged_all <= gnomadCutoff) & # @arg : gnomadCutoff
                            (mt.info.MQ >= 40))
        mt = mt.annotate_entries(PLDIFF = hl.sorted(mt.PL)[1] - hl.sorted(mt.PL)[0]) # annotate PLDIFF
        mt = mt.annotate_entries(PLDIFFDP = mt.PLDIFF / mt.DP) # annotate PLDIFFDP

        mt = mt.filter_entries((mt.GQ > GQCutoff) & # @arg : GQCutoff
                               (mt.DP >= DPCutoff) & # @arg : DPCutoff
                               (mt.GT.is_het()) & # dominant model
                               (mt.PLDIFFDP >= PLDIFFDPCutoff) & # @arg : PLDIFFDPCutoff
                               (mt.PL[1] == 0), keep = True) # non normalized PL
        return mt
    ...
    # main function 
    def annovar_vcf_preprocess(self, path1, lcr_bed, gnomadCutoff, GQCutoff, DPCutoff, PLDIFFDPCutoff): 
        mt = self.import_myvcf(path1)
        mt = self.lcr_filter(mt, lcr_bed)
        mt = self.mt_unkey(mt)
        path2 = path1.replace(".vcf", "_variant.table")
        gnomadCutoff = float(gnomadCutoff)
        GQCutoff = int(GQCutoff)
        DPCutoff = int(DPCutoff)
        PLDIFFDPCutoff = int(PLDIFFDPCutoff)
        ...
        mt = self.filtering_myvcf(mt, gnomadCutoff, GQCutoff, DPCutoff, PLDIFFDPCutoff)
        mt = self.select_rows_from_mt(mt)
        self.export_hailtable(mt, path2)

myobj = AnnovarWithHail()
myobj.annovar_vcf_preprocess(input_file, lcr_bed, MAF, 20, 8, 8)
hl.stop()
