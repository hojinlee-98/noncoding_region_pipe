...
setwd("/Volumes/hjdrive/thyroiditis/thyroiditis_noncoding/enhancer_promoter/v8/results/20230102/")

library(tidyverse)
library(dplyr)
source("/Volumes/hjdrive/thyroiditis/thyroiditis_noncoding/enhancer_promoter/v8/scripts/20221230/dom/multigene_process_Dec312022_hj.R") # expand_df
source("/Volumes/hjdrive/thyroiditis/thyroiditis_noncoding/enhancer_promoter/v8/scripts/20221230/dom/manta_annotsv_varianttable_Jan022023_hj.R") # manta_concat
source("/Volumes/hjdrive/thyroiditis/thyroiditis_noncoding/enhancer_promoter/v8/scripts/20221230/dom/geneCounts_varianttable_Jan022023_hj.R") # count_table
...
### file name ###
filename <- paste0('thy_n17_case_method11_', format(MAF, scientific=FALSE), '_asian_', format(asian, scientific=FALSE))
...
### subset table 
cases[c("encode_PLS", "encode_pELS", "encode_dELS", "PLS_ELS")] <- NA_character_ # add empty field
cases <- cases %>% select('Gene.refGeneWithVer', 'SAMPLE','AD', 'CHROM', 'POSITION', 'REFERENCE.ALLELE', 'ALTERNATE.ALLELES', 'AAChange.refGeneWithVer', 'Func.refGeneWithVer', 'ExonicFunc.refGeneWithVer', 'gnomAD_r2.1.1_merged_all', 'bravo_freeze8', 'gnomad_exome_AF_eas', 'gnomad_genome_AF_eas', 'MetaSVM_pred', 'REVEL_rankscore', 'CADD13_PHRED.1', 'spliceAI_DS_AG.1', 'spliceAI_DS_AL.1', 'spliceAI_DS_DG.1', 'spliceAI_DS_DL.1', 'pLi_gnomad.refGeneWithVer', 'oe_lof_upper_gnomad.refGeneWithVer', 'mis_z_gnomad.refGeneWithVer', 'oe_mis_upper_gnomad.refGeneWithVer', "encode_PLS", "encode_pELS", "encode_dELS", "PLS_ELS", "GT", "DP", "GQ", "MQ")
colnames(cases) <- c("Gene", "Sample", "AD", "CHROM", "POSITION", "REF", "ALT", "AAChange", "func.region", "VariantClass", "gnomAD", "bravo", "gnomAD.eas.exome", "gnomAD.eas.genome", "MetaSVM", "REVEL", "CADD13", 'spliceAI_AG', 'spliceAI_AL', 'spliceAI_DG', 'spliceAI_DL', "pLI", "LOEUF", "mis_z", "MOEUF", "PLS", "pELS", "dELS", "PLS_ELS", "GT", "DP", "GQ", "MQ")
...
### functional region ###
cases_noncoding <- cases_noncoding %>%
  filter(!is.na(encode_PLS) | !is.na(encode_pELS) | !is.na(encode_dELS)) %>% 
  mutate(PLS_ELS = case_when(
    !is.na(encode_PLS) ~ encode_PLS,
    !is.na(encode_pELS) ~ encode_pELS,
    !is.na(encode_dELS) ~ encode_dELS,
    TRUE ~ NA_character_))
...
cases_indel <- indel_finder(cases_noncoding) # indel for PLS, pELS, and dELS

cases_noncoding$jarvis <- as.numeric(cases_noncoding$jarvis)
cases_noncoding <- cases_noncoding %>% filter((jarvis >=0.9) & !(is.na(jarvis))) # snp for PLS, pELS, and dELS
# snp + indel 
...
filelist_ins <- read.table("/Volumes/hjdrive/thyroiditis/thyroiditis_sv/manta_geneCounts/20221228_manta_annotsv_dom.ins.list.txt", sep = "\t", quote = "")[[1]]
# make object for manta deletion 
cases_sv <- rbind(manta_concat(filelist), manta_concat(filelist_ins))
# total alleles for dominant
N <- 17*2 # het
### filtering ###
cases_sv <- cases_sv %>% filter((X1000g_AF <= MAF) & (IMH_AF <= MAF) & (SR_fraction >= 0.4)) %>% mutate(POSITION = paste(intersectStart, intersectEnd, sep = "_"))
cases_sv.subset <- cases_sv %>%
  filter(AnnotSV.ranking == 5) %>%
  select(Gene.name, Sample, SV.chrom, POSITION, REF, ALT, SV.type, GT, GQ, SR_fraction, X1000g_AF, IMH_AF, pLI_ExAC, AnnotSV.ranking) %>%
  rename(Gene = Gene.name, CHROM = SV.chrom, VariantClass = SV.type) %>%
  mutate(split.Gene = Gene) %>%
  transform(GQ = as.numeric(GQ))

cases <- bind_rows(cases, cases_sv.subset)

### geneCounts
count.table <- count_table(cases, "split.Gene")
cname <- c("mut", "nor")
