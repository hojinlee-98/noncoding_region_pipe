...
### file name ###
filename <- paste0('thy_n17_case_method6_', format(MAF, scientific=FALSE), '_asian_', format(asian, scientific=FALSE))
...
cases_indel <- indel_finder(cases_noncoding) # indel for PLS and pELS

cases_noncoding$jarvis <- as.numeric(cases_noncoding$jarvis)
cases_noncoding <- cases_noncoding %>% filter(!(is.na(encode_PLS)) & (jarvis >=0.9) & !(is.na(jarvis))) # snp for only PLS
# snp + indel 
