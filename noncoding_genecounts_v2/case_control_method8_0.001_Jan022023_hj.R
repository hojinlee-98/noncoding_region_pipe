...
### file name ###
filename <- paste0('thy_n17_case_method8_', format(MAF, scientific=FALSE), '_asian_', format(asian, scientific=FALSE))
...
cases_noncoding <- cases_noncoding %>%
  filter(!is.na(encode_PLS) | !is.na(encode_pELS)) %>%
  filter((jarvis >=0.9) & !(is.na(jarvis))) # snp for PLS and pELS
# snp + indel 
